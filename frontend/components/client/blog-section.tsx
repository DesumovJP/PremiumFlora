...
export function BlogSection({ posts }: BlogSectionProps) {
  ...
  return (
    <>
      <section ref={ref} className="relative overflow-hidden py-20 lg:py-28 bg-gradient-to-b from-white via-amber-50/20 to-emerald-50/30">
          ...
          >
            ...
            <div className="flex gap-3 overflow-x-auto pb-4 px-4 sm:hidden snap-x snap-mandatory scroll-pl-4 scrollbar-hide">
              {featuredPosts.map((post, index) => (
                ...
                >
                  <Card
                    className="group h-full flex flex-col overflow-hidden border border-slate-100 bg-white cursor-pointer rounded-xl active:scale-[0.98] transition-transform duration-150"
            ...
            <div className="hidden sm:grid sm:grid-cols-2 lg:grid-cols-3 gap-6 lg:gap-8">
              {featuredPosts.map((post, index) => (
                ...
                >
                  <Card
                    className="group h-full flex flex-col overflow-hidden border border-slate-100 bg-white cursor-pointer rounded-2xl transition-colors duration-200 hover:border-emerald-200"
                    onClick={() => setSelectedPost(post)}
                  >
                    {/* Image */}
                    <div className="relative aspect-[16/10] w-full overflow-hidden bg-slate-100">
          ...
          >
            ...
            >
              <Button
                asChild
                size="lg"
                className="group relative overflow-hidden bg-white hover:bg-amber-50 border-2 border-slate-200 hover:border-amber-200 text-slate-700 hover:text-amber-700 shadow-sm hover:shadow-md h-12 sm:h-14 px-8 text-base font-semibold transition-all duration-300"
              >
