    ...
    >
      ...
      >
        <div className="max-w-4xl mx-auto text-center">
          ...
          {/* Heading */}
          <h2 className="text-[14px] sm:text-lg md:text-xl lg:text-2xl xl:text-3xl font-bold text-slate-900 dark:text-white mb-1.5 sm:mb-4 leading-tight">
            {c.title}{' '}
            <span className="text-emerald-600 dark:text-emerald-400">{c.titleAccent}</span>
          </h2>

          {/* Subtitle */}
          <p className="text-[11px] sm:text-xs md:text-sm lg:text-base text-slate-600 dark:text-slate-400 mb-5 sm:mb-8 max-w-2xl mx-auto">
            {c.subtitle}
          </p>
          ...
          <div className="flex flex-col sm:flex-row items-center justify-center gap-3 sm:gap-4">
            <Button
              ...
              className="group w-auto h-9 sm:h-10 md:h-12 px-4 sm:px-5 md:px-6 bg-emerald-600 hover:bg-emerald-700 text-white font-semibold text-[12px] sm:text-[13px] md:text-sm rounded-full transition-all duration-300 hover:shadow-lg hover:shadow-emerald-500/25"
            >
              {c.buttonText}
              <ArrowRight className="w-3.5 h-3.5 sm:w-4 sm:h-4 md:w-4 md:h-4 ml-1.5 sm:ml-2 transition-transform group-hover:translate-x-1" />
            </Button>
